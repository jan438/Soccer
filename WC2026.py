...
for poule in range(12):
    ...
    for team in range(teamspp):
        ...
        if nationsdata[teamcounter][8] != "NL":
            ...
            renderPDF.draw(drawing, my_canvas, float(nationsdata[teamcounter][6]), float(nationsdata[teamcounter][7]) - 50)
            my_canvas.setFillColor(HexColor("#ffff00"))
            my_canvas.circle(float(nationsdata[teamcounter][6]), float(nationsdata[teamcounter][7]) - 50, 2.0, stroke = 0, fill = 1)
